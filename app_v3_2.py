...
from saved_models.model import LSTMmodel, NeuralModel, GRUmodel, RNNmodel, LSTMNet
import torch
import cv2
import dlib
...
LSTM_model_that_works_with_136_features = False # True
LSTM_model_that_works_with_raw_images = True # False
model = None
model2 = None
...
detector = dlib.get_frontal_face_detector()  # dlib face detection object (replace with Face Detection Team's code)
predictor = dlib.shape_predictor(
    "shape_predictor_68_face_landmarks.dat")  # initalise shape pred object & load pre trained model
frames_cap = []  # initialise empty list for saved frames
keypoints = []  # initialise empty list for shape and keypoint coordinates for corresponding frames
...
def pre_process_image(img):
    global X_test, cap, detector, predictor, frames_cap, keypoints, VGG
    if LSTM_model_that_works_with_raw_images:

        xx_data = cv2.resize(img, dsize=(128, 128), interpolation=cv2.INTER_CUBIC)
        ...
        # xx_data = xx_data[:16, :16, 0:2].reshape((1, 512))
        # yy_data = xx_data.copy()
        # xx_data = np.concatenate((xx_data, yy_data), axis=0)
        X_test = torch.Tensor(
            features
        )
        # print(X_test.shape)
        # exit()
    elif LSTM_model_that_works_with_136_features:
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # convert frame to grayscale to speed up processing

        faces = detector(gray)  # detect faces in the frame (redundant?)

        data = []  # initialise empty list for the face keypoint coordinates
        keypoints = []
        # iterate over faces detected (will just be one iteration since only one face is detected)
        for face in faces:
            # get face box coordinates
            ## replace with face detection function here? ##
            x1 = face.left() - 25
            y1 = face.top() - 25
            x2 = face.right() + 25
            y2 = face.bottom() + 50  # these +50/25s are arbitrary to make sure the face fits the box even if mouth open wide

            # add the width and height of the face box to the keypoint data list
            data.append(x2 - x1)
            data.append(y2 - y1)
            ...
            # get keypoint coordintes
            landmarks = predictor(gray, face)

            # iterate ove coordinates
            for n in range(0, 67):
                x = landmarks.part(n).x
                y = landmarks.part(n).y

                data.append(x - x1)  # append x coord relative to face box
                data.append(y - y1)  # append y coord relative to face box

            # draw keypoints on the frame
            # cv2.circle(frame, (x,y), 6, (255, 0, 0), -1)

            # rescale cooridnates to
            # data = rescale_landmarks(data)

            # add the first frame as a control
            if len(keypoints) == 0:
                keypoints.append(data)
                frames_cap.append(
                    transform.resize(img[y1:y2, x1:x2], (512, 512)))  # note y and x coords are inversed since
            # matrices are (rows, columns) i.e. (y,x)

            # after the first frame, check if it's much different than previous frame, if so save it
            if len(keypoints) > 0:
                # check mse between the previously saved frame
                if True or check_mse(keypoints[-1],
                                     data) > 30:  # 30 is arbitrary and needs more testing (angry face isnt much different than neutral)
                    frames_cap.append(transform.resize(img[y1:y2, x1:x2], (512, 512)))  # storing just the face
                    keypoints.append(
                        data)  # save keypoint coordinates (will be replaced by return if this is wrapped in function)
...
def do_predict(land_marks):
    #
    global framework, test_size, batch_size, X_test, prediction
    test_size = X_test.shape[0]
    # get_model()
    # model2 = custom_load_model(export_path2)
    if LSTM_model_that_works_with_raw_images:
        seq_len = 16
        # print(X_test.shape)
        print(X_test.shape[1] // seq_len)
        # exit()
        export_path2 = "./saved_models/LSTM_model_that_works_with_raw_images/model-ckpt-best.txt"
        ## LSTM (input, hidden, seq_len, output)
        framework = LSTMmodel(X_test.shape[1] // seq_len, 35, seq_len, 2)
    elif LSTM_model_that_works_with_136_features:
        export_path2 = "./saved_models/LSTM_model_that_works_with_136_features/model-ckpt-best.txt"
        # export_path2 = "model-ckpt-best.txt"
        seq_len = 2
        # print(X_test.shape[1] // seq_len)
        # exit()
        # return [11, X_test.shape[1]]
        framework = LSTMmodel(X_test.shape[1] // seq_len, 136, seq_len, 2)
    optimizer = torch.optim.Adam([{'params': framework.parameters()}], lr=0.0001)
    msg = 'test'
    framework.load_state_dict(torch.load(export_path2))
    batch_size = 1
    # framework.load_state_dict(torch.load("./Model_LSTM_new/model-ckpt-best.txt"))
    ...
    # print(prediction)
    prediction = torch.stack(prediction).squeeze(dim=1)
    print("Valence : ", float(prediction[0, 0]))
    print("Arousal : ", float(prediction[0, 1]))
...
def handle_my_custom_event(arg1):
    ...
    fh.close()
    ##################
    # img_f = cv2.imread(temp_file_name)
    # land_marks = pre_process_image(cv2.imread(temp_file_name))
    test = do_predict(pre_process_image(cv2.imread(temp_file_name)))
    ...
    # print(record_result)
    ###############################
    # output_new = custom_prediction()
    emit('new_message_client_temp',
         {
             'error': 'None',
             'response': {
                 'labels': copy_pred,
                 'arousal': test[1],  # float(output_new[0, 1]),
                 'valence': test[0]  # float(output_new[0, 0])
             }
         })
...
def custom_load_model(model_path):
    global framework, LSTM_model_that_works_with_136_features, LSTM_model_that_works_with_raw_images, X_test
    seq_len = 2
    framework = LSTMmodel(X_test.shape[1] // seq_len, 136, seq_len, 2)
    optimizer = torch.optim.Adam([{'params': framework.parameters()}], lr=0.0001)
...
def custom_prediction():
    global framework, test_size, batch_size, X_test
    batch_size = 1
    for param in framework.parameters():
        param.requires_grad = False
    ...
    # plot_two(predictions[:, 0], "Valence")
    # plot_two(predictions[:, 1], "Arousal")
    return predictions
...
def get_model():
    ...
    global model
    global model2
    global LSTM_model_that_works_with_136_features
    global LSTM_model_that_works_with_raw_images
    if model == None:
...
if __name__ == '__main__':
    ...
    # temp_file_name = './static/img/test/1619920173.249599.jpg'
    # test = do_predict(pre_process_image(cv2.imread(temp_file_name)))
